...
const buildData = async ({ start, end }) => {
    ...
    const logs = (await StudentAcademicCourseLog.findAll({
        where: {
            createdAt: {
                [Op.gte]: start,
                [Op.lt]: end
...
module.exports = async ({ start, end }) => {
    console.log('Academic Course Log Cron Job Started...', start, end)
    try {
        await buildData({ start, end })
        console.log('Academic Course Log Cron Job Finished...')
    } catch (err) {
        console.error('Academic Course Log Cron Job Error...')
