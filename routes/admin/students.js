...
const { College, User, Specialization, CourseAssignedToStudent, AcademicCourse, AcademicCourseAssignedToStudent, LoginReport } = require('../../models')
...
router.get('/', async (req, res) => {
    try {
        ...
        if (req.query.college)
            conditions.college = req.query.college

        if (req.query.specialization)
            conditions.specialization = req.query.specialization

        if (req.query.semester)
            conditions.semester = req.query.semester

        if (req.query.enrollmentYear)
            conditions.enrollmentYear = req.query.enrollmentYear

        if (req.query.gender)
            conditions.gender = req.query.gender

        const [students, Colleges, Specializations] = await async.parallel([
        ...
        res.render('admin/students', { User: req.user, students, colleges, Colleges, specializations, Specializations })
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        res.render('admin/student', { User: req.user, student: foundStudent, college: foundCollege, specializations, formatDate, courses: result, previousCourses: result2, academicCourses: result3, previousAcademicCourses: result4, formatDateMoment, lastLogin })
    } catch (err) {
        console.error('\x1b[31m%s\x1b[0m', err)
...
router.get('/:id/progress', async (req, res) => {
    try {
        if (req.query.courseType === 'ibm') {
            ...
            res.render('admin/StudentProgress', { User: req.user, CourseDetails, formatDateMoment })
            ...
            res.render('admin/studentAcademicCourseProgress', { User: req.user, course: academicCourse, viewCourse, formatDateMoment })
        } else {
