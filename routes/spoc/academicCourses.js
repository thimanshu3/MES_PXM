...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (!ac) {
            ...
            res.redirect('/spoc/academicCourses')
            return
        }
        ...
        res.render('spoc/academicCourse', { User: req.user, course: ac, formatDateMoment })
    } catch (err) {
        console.error('\x1b[31m%s\x1b[0m', err)
        req.flash('error', 'Something Went Wrong!')
        res.redirect('/')
...
router.get('/:id/:chapterId/:lessonId', async (req, res) => {
    try {
        ...
        res.render('spoc/academicCourseLesson', { User: req.user, lesson })
    } catch (err) {
        console.error('\x1b[31m%s\x1b[0m', err)
        req.flash('error', 'Something Went Wrong!')
        res.redirect('/')
