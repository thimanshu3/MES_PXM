...
const { College, User, Specialization, CourseAssignedToStudent, FacultySemesterAssignment, FacultySpecializationAssignment, AcademicCourseAssignedToStudent, AcademicCourse, LoginReport } = require('../../models')
...
router.get('/', async (req, res) => {
    try {
        ...
        res.render('faculty/students', { User: req.user, students, specializations })
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        res.render('faculty/student', { User: req.user, student: foundStudent, college: foundCollege, specializations, formatDate, courses: result, previousCourses: result2, academicCourses: result3, previousAcademicCourses: result4, formatDateMoment, lastLogin })
    } catch (err) {
        console.error('\x1b[31m%s\x1b[0m', err)
        req.flash('error', 'Something Went Wrong!')
        res.redirect('/')
...
router.get('/:id/progress', async (req, res) => {
    try {
        if (req.query.courseType === 'ibm') {
            ...
            res.render('faculty/StudentProgress', { User: req.user, CourseDetails, formatDateMoment })
            ...
            res.render('faculty/studentAcademicCourseProgress', { User: req.user, course: academicCourse, viewCourse, formatDateMoment })
        } else {
